...
public class PButton extends AppCompatButton implements View.OnTouchListener {
    ...
    private void init() {
        ...
        mButtonColor = resources.getColor(R.color.pbutton_default_color);
        mShadowColor = resources.getColor(R.color.pbutton_default_shadow_color);
        mShadowHeight = resources.getDimensionPixelSize(R.dimen.pbutton_default_shadow_height);
        mCornerRadius = resources.getDimensionPixelSize(R.dimen.pbutton_default_conner_radius);
    }

    private void parseAttrs(Context context, AttributeSet attrs) {
        //Load from custom attributes
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.pbutton);
        if (typedArray == null) return;
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.pbutton_shadowEnabled) {
                isShadowEnabled = typedArray.getBoolean(attr, true); //Default is true
            } else if (attr == R.styleable.pbutton_buttonColor) {
                mButtonColor = typedArray.getColor(attr, R.color.pbutton_default_color);
            } else if (attr == R.styleable.pbutton_shadowColor) {
                mShadowColor = typedArray.getColor(attr, R.color.pbutton_default_shadow_color);
                isShadowColorDefined = true;
            } else if (attr == R.styleable.pbutton_shadowHeight) {
                mShadowHeight = typedArray.getDimensionPixelSize(attr, R.dimen.pbutton_default_shadow_height);
            } else if (attr == R.styleable.pbutton_cornerRadius) {
                mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.pbutton_default_conner_radius);
            }
        }
        typedArray.recycle();
    ...
    public void setpbuttonPadding(int left, int top, int right, int bottom) {
        mPaddingLeft = left;
        mPaddingRight = right;
        mPaddingTop = top;
